...
class WonksNetService:
    ...
    def predict(cls, cropped_rgb, scope=constant.DB_PH_SCOPE, k=10, sort_by="confidence", categories=None):
        ...
        for engine in targets:
            # engine.search now returns our fully loaded dictionaries containing the masks!
            ...
            if final_forensic is None and forensic is not None:
                ...
                # Aligning this with the 'orb_similarity' key we set in the updated search engine
                if res and 'orb_similarity' in res[0]:
                    max_orb_score = res[0]['orb_similarity']
        ...
        # 🟢 NEW: Guarantee the visual placeholder keys exist for the API endpoint
        # This acts as a safety net so the Flask route never crashes on a missing key
        for item in top_k_results:
            item.setdefault('attention_mask', None)
            item.setdefault('pixel_dna', None)
            item.setdefault('forensic_viz', None)
            item.setdefault('orb_similarity', 0.0)
